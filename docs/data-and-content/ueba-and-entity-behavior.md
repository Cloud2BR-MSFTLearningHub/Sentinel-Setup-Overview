...
- [UEBA in Microsoft Sentinel](https://learn.microsoft.com/en-us/azure/sentinel/identify-threats-with-entity-behavior-analytics)
...
building profiles for users, hosts, IP addresses, and other entities. It improves
...
- Map account, host, IP, URL, cloud application, and resource entities consistently
  in analytics rules.
- Resolve naming differences across systems before correlating them. A display
  name alone is not a reliable identity key.
- Restrict UEBA access and data handling to the same privacy and employment-policy
...
3. Confirm account, device, IP, and time relationships with stable identifiers.
